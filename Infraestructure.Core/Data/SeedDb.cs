...
namespace Infraestructure.Core.Data
{
    public class SeedDb
    {
        ...
        {
            //asegurarse que la base de datos este creada
            ...
            //await = espera que finalice un proceso para que siga con el siguiente
        }


        private async Task CheckTypeStateAsync()
        {
            if (!_context.TypeStateEntity.Any())// any es si contiene registros
            {
        ...
        {
            if (!_context.TypePermissionEntity.Any()) //comprobar que si no hay tipos de permisos entonces los cree
            {
